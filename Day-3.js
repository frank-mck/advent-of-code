...
const getSymbols = (string) => {
  return [...string.matchAll(/[^a-zA-Z0-9._]/gi)];
};

const getNumbers = (string) => {
  return [...string.matchAll(/[0-9]+/gi)];
};

function findIndexes(numbers, symbols, i) {
  const nextLineIndex = i * 140;
  const numberIndexe = [];
  const symbolIndexe = [];

  numbers.map((num) => {
    const id = (((1 + Math.random()) * 70653344226600) | 5)
      .toString(16)
      .substring(2);
    ...
    for (let index = 0; index < numSize; index++) {
      const patNumIndex = num.index + +nextLineIndex + +index;
      numberIndexe.push({ [patNumIndex]: num[0], id: id });
    }
  });

  symbols.map((symbol) => {
    const ids = (((1 + Math.random()) * 70653344226600) | 5)
      .toString(16)
      .substring(2);

    symbolIndexe.push({ index: symbol.index + +nextLineIndex, id: ids });
  });

  numberIndexes.push(numberIndexe);
  symbolIndexes.push(symbolIndexe);
}
for (let i = 0; i < dataArray.length; i++) {
  const currentLine = dataArray[i];
  const numbersCurrentLine = getNumbers(currentLine);
  const symbolsCurrentLine = getSymbols(currentLine);

  let numbersNextLine, numbersPrevLine, symbolsNextLine, symbolsPrevLine;

  if (i > 0) {
    symbolsPrevLine = getSymbols(dataArray[i - 1]);
    numbersPrevLine = getNumbers(dataArray[i - 1]);
  }

  if (i + 1 < dataArray.length) {
    symbolsNextLine = getSymbols(dataArray[i + 1]);
    numbersNextLine = getNumbers(dataArray[i + 1]);
  }

  findIndexes(numbersCurrentLine, symbolsCurrentLine, i);
}

const numberPositions = numberIndexes.flat();
const symbolPositions = symbolIndexes.flat();

numberPositions.map((num, index) => {
  const partId = Object.values(num)[1];
  const number = Object.keys(num)[0];
  const alreadyThere = partNumbers.some((part) => part.id === partId);

  symbolPositions.some((sym) => {
    if (
      (+sym.index === +number + 1 && !alreadyThere) ||
      (+sym.index === +number - 1 && !alreadyThere) ||
      (+sym.index === +number + 139 && !alreadyThere) ||
      (+sym.index === +number + 140 && !alreadyThere) ||
      (+sym.index === +number - 141 && !alreadyThere) ||
      (+sym.index === +number - 140 && !alreadyThere) ||
      (+sym.index === +number + 141 && !alreadyThere) ||
      (+sym.index === +number - 139 && !alreadyThere)
    ) {
...
console.log(
  partNumbers
    .map((partNum) => Object.values(partNum)[0])
    .reduce((calc, sum) => +calc + +sum)
);

const symbolMap = {};

partNumbers.forEach((obj) => {
  if (!symbolMap[obj.symbol]) {
    symbolMap[obj.symbol] = { count: 1, product: +Object.values(obj)[0] };
  } else {
    symbolMap[obj.symbol].count += 1;
    symbolMap[obj.symbol].product *= +Object.values(obj)[0];
  }
});

const filteredData = Object.values(symbolMap).filter((g) => g.count >= 2);

let result = 0;

for (let i = 0; i < filteredData.length; i++) {
  result += filteredData[i].product;
}
